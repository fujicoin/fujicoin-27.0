...
public:
    ...
    void setDisplayUnit(FujicoinUnit unit)
    {
        bool valid = false;
        CAmount val = value(&valid);
    ...
    QSize minimumSizeHint() const override
    {
        if(cachedMinimumSizeHint.isEmpty())
        {
            ...
            int w = GUIUtil::TextWidth(fm, FujicoinUnits::format(FujicoinUnit::FJC, FujicoinUnits::maxMoney(), false, FujicoinUnits::SeparatorStyle::ALWAYS));
...
private:
    FujicoinUnit currentUnit{FujicoinUnit::FJC};
...
void FujicoinAmountField::unitChanged(int idx)
{
    ...
    QVariant new_unit = unit->currentData(FujicoinUnits::UnitRole);
    assert(new_unit.isValid());
    amount->setDisplayUnit(new_unit.value<FujicoinUnit>());
}

void FujicoinAmountField::setDisplayUnit(FujicoinUnit new_unit)
{
