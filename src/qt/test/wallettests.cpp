// Copyright (c) 2015-2022 The Fujicoin Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<FujicoinAmountField*>("payAmount")->setValue(amount);
...
{
    FujicoinUnit unit = walletModel.getOptionsModel()->getDisplayUnit();
    QString balanceComparison = FujicoinUnits::formatWithUnit(unit, expected_balance, false, FujicoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_fujicoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_fujicoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_fujicoin-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    FujicoinAmountField* amountInput = receiveCoinsDialog.findChild<FujicoinAmountField*>("reqAmount");
