...
To run the unit tests manually, launch `src/test/test_fujicoin`. To recompile
...
To run the GUI unit tests manually, launch `src/qt/test/test_fujicoin-qt`
...
`test_fujicoin` accepts the command line arguments from the boost framework.
...
test case is entered, for example. `test_fujicoin` also accepts the command
line arguments accepted by `fujicoind`. Use `--` to separate both types of
arguments:

```bash
test_fujicoin --log_level=all --run_test=getarg_tests -- -printtoconsole=1
```
...
is happening by running `gdb ./src/test/test_fujicoin` and then using the `bt` command
within gdb.
...
```bash
gdb src/test/test_fujicoin core
