...
application bridge enabled. The following routers are recommended for use with Fujicoin Core:
...
Once an I2P router with SAM enabled is up and running, use the following Fujicoin
...
debug log about your I2P configuration and connections. Run `fujicoin-cli help
...
The first time Fujicoin Core connects to the I2P router, it automatically
...
`i2p_private_key` in the Fujicoin Core data directory.  The persistent I2P
...
## Fetching I2P-related information from Fujicoin Core

There are several ways to see your I2P address in Fujicoin Core if accepting
...
You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `fujicoin-cli help getnodeaddresses` for details.
...
It's important that the nodes of a popular application like Fujicoin contribute
...
## Bundling I2P in a Fujicoin application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with Fujicoin.
