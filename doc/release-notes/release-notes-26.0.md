...
using the Linux kernel, macOS 11.0+, and Windows 7 and newer.  Fujicoin
...
- `fujicoind` and `fujicoin-qt` will now raise an error on startup
 if a datadir that is being used contains a fujicoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a fujicoin.conf file. The error message is just a diagnostic intended
 ...
 the fujicoin.conf contained in it. (#27302)
...
- A new `fujicoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `fujicoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Fujicoin Core and import the data into a new wallet.
...
```sh
src/fujicoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/fujicoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Fujicoin Core would
...
- Bash completion files have been renamed from `fujicoin*.bash-completion` to
  `fujicoin*.bash`. This means completions can be automatically loaded on demand
