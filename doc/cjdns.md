# CJDNS support in Fujicoin Core

It is possible to run Fujicoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Fujicoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Fujicoin network and individual nodes.
...
before it will work with your Fujicoin Core node. This is described in steps
...
## Run Fujicoin Core with CJDNS

Once you are connected to the CJDNS network, the following Fujicoin Core
...
When enabled, this option tells Fujicoin Core that it is running in an
...
IPv6 local network. This helps Fujicoin Core perform better address management:
...
CJDNS support was added to Fujicoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `fujicoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/fujicoin/fujicoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Fujicoin Core

There are several ways to see your CJDNS address in Fujicoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `fujicoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `fujicoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `fujicoin-cli help getnodeaddresses` for details.
