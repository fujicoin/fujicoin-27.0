    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            for node in legacy_only_nodes:
                # RPC loadwallet failure causes fujicoind to exit in <= 0.17, in addition to the RPC
        ...
        for wallet_name in ["w1", "w2", "w3"]:
            if self.options.descriptors:
                ...
                node_v16.assert_start_raises_init_error([f"-wallet={wallet_name}"], f"Error: Error loading {wallet_name}: Wallet requires newer version of Fujicoin Core")
        ...
        for node in descriptors_nodes if self.options.descriptors else legacy_nodes:
            ...
            # Make sure this wallet opens with only the migration warning. See https://github.com/fujicoin/fujicoin/pull/19054
