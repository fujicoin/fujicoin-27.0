...
# TestP2PConn: A peer we use to send messages to fujicoind, and store responses.
...
class CompactBlocksTest(FujicoinTestFramework):
    ...
    # This test actually causes fujicoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that fujicoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for fujicoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # fujicoind will not send blocktxn responses for blocks whose height is
