#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Fujicoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing fujicoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='fujicoin-cli', help='fujicoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Fujicoin address to which the faucet should send')
...
parser.add_argument('fujicoin_cli_args', nargs='*', help='Arguments to pass on to fujicoin-cli (default: -signet)')

args = parser.parse_args()

if args.fujicoin_cli_args == []:
    args.fujicoin_cli_args = ['-signet']


def fujicoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.fujicoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = fujicoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = fujicoin_cli(['getnewaddress', 'faucet', 'bech32'])
