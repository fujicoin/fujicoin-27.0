...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Fujicoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"fujicoin_verify_binaries.{args.version}"
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the fujicoin release to download; of the format '
